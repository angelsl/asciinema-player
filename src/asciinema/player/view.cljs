...
(defn part-props [{:keys [inverse cursor] :as attrs}]
  (let [inverse (if cursor (not inverse) inverse)
    ...
    (merge (when class-name {:class class-name})
...
(defn logo-play-icon []
  [:svg {:version "1.1" :view-box "0 0 866.0254037844387 866.0254037844387" :class "icon" :dangerouslySetInnerHTML {:__html logo-raw-svg}}])

(defn play-icon []
  [:svg {:version "1.1" :view-box "0 0 12 12" :class "icon"}
  ...
  (let [mouse-moves-ch (chan (dropping-buffer 1))
    ...
    (fn []
      [:div.asciinema-player-wrapper {:tab-index -1
       ...
       [:div.asciinema-player {:class @player-class-name
